 ...
 */
export const getRocketStimuli = (
  isTraining: boolean,
  swapSides: boolean,
): { leftStimulus: string; rightStimulus: string } => {
  if (isTraining) {
    return swapSides
...
export const getPlanetStimulus = (planet: PlanetType): string => {
  switch (planet) {
    case PlanetType.RED:
      return 'main_planet_red.png';
    case PlanetType.PURPLE:
      return 'main_planet_purple.png';
    case PlanetType.GREEN:
      return 'tutorial_planet_green.png';
    case PlanetType.YELLOW:
      return 'tutorial_planet_yellow.png';
    default:
      throw new Error(`Unknown 'PlanetType': ${planet}`);
  }
};
...
const createTimeline = (): any[] => {
  ...
  timeline.push({
    ...
    question: {
      prompt:
        'If an alien shares a space resource in this mission,<br>then they will probably have space resources to share in the next few missions.',
      correct: 'true',
    },
    preamble: 'Please answer the following question:',
    button_label: 'Continue',
    feedback:
      'The amount of resources an alien can share changes slowly over multiple missions,<br>so they will probably have space resources to share in the next few missions.',
    extensions: [
      {
        type: NeurocogExtension,
      },
...
const validateTimeline = (timeline: any[]) => {
  ...
  if (filteredTimeline.length !== expectedTrainingTrialsLength) {
    logger.warn(
      `Timeline is not the expected length of ${expectedTrainingTrialsLength}! Actual: ${filteredTimeline.length}`,
    );
  } else {
  ...
  if (filteredTimeline.length !== expectedCommonTransitionMainTrials) {
    logger.warn(
      `Timeline does not include expected number of common transition trials (${expectedCommonTransitionMainTrials})! Actual: ${filteredTimeline.length}`,
    );
  } else {
    logger.success(
      `Timeline includes expected number of common transition trials: ${expectedCommonTransitionMainTrials} trials`,
    );
  }
};
