let block_trials = 50; // total number of trials in each block
let transprob = 0.7; // probability of 'correct' transition

let right_key = 48; // 0 at top of keyboard
let left_key = 49; // 1 at top of keyboard

let randomize_side = false; // whether we should further randomize robot sides

// subject level display orders for when we aren't randomizing every trial
let rocket_sides = (Math.random() < .5);
let prac_rocket_sides = (Math.random() < .5);
let red_display_order = (Math.random() < .5);
let purple_display_order = (Math.random() < .5);
let green_display_order = (Math.random() < .5);
let yellow_display_order = (Math.random() < .5);

let moneytime = 1000; // 1000 according to Decker 2016;
let isitime = 1000; // 1000 according to Decker 2016;
let choicetime = 3000; // 3000 according to Decker 2016;
let box_moving_time = 90; // from comment in matlab code
let ititime = 1000; // 1000 according to Decker 2016;

let num_blocks = 4;

let reward_instructions_payoff = ['.8', '.8', '.8', '.8'];
let instructions_payoff = ['.9', '.1', '.9', '.1'];

let red_planet_first_rocket = (Math.random() < 0.5);

let probability_file = 'fixed/masterprob4.csv';
let practice_probability_file = 'fixed/masterprobtut.csv';

let reward_string = 'images/t.png';
let null_string = 'images/nothing.png';

let practice_reward = 0;
let real_reward = 0;

let practice_pressing_idx = 5;
let practice_pressing_num = 4; // 4 trials to practice selecting alien
let practice_reward_idx = 10 + practice_pressing_num;
let practice_reward_num = 10; // 10 trials to practice asking alien for treasure
let practice_stochastic_idx = 17 + practice_pressing_num + practice_reward_num;
let practice_stochastic_num = 10; // 10 trials practice choosing between aliens
let practice_game_idx =
    34 + practice_pressing_num + practice_reward_num + practice_stochastic_num;
let practice_game_num = 20; // 20 trials to practice full game
...
let all_images = image_strings.map((data) => {
...
let monster_size = picture_height * 300 / 758;
// similarly for the reward
let reward_size = picture_height * 75 / 758;

let x_center = width / 2;
let y_center = height / 2;

let choice_y = y_center + 0.22 * picture_height - monster_size / 2 ;
let choice_x_right = x_center + 0.25 * picture_width - monster_size / 2;
let choice_x_left = x_center - 0.25 * picture_width - monster_size / 2;

let chosen_y = y_center - 0.06 * picture_height - monster_size / 2;
let chosen_x = x_center - monster_size / 2;

let reward_y = y_center - 0.06 * picture_height - reward_size / 2 - monster_size / 2;
let reward_x = x_center - reward_size / 2;

let text_start_y = y_center - 0.2 * picture_height;
let instructions_text_start_y = y_center - 0.4 * picture_height;
let text_start_x = x_center - 0.49 * picture_width;
let font_size = picture_height * 25 / 758;

let instructions_backgrounds = [
