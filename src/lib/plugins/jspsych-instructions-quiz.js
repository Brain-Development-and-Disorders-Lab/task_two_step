...
jsPsych.plugins['instructions-quiz'] = (function() {
  ...
  plugin.info = {
    ...
    parameters: {
      ...
      trial_ends_after_audio: {
        ...
        default: false,
        description: 'If true, then the trial will end as soon ' +
            'as the audio file finishes playing.',
      },
      feedback_duration: {
        type: jsPsych.plugins.parameterType.INT,
  ...
  plugin.trial = function(display_element, trial) {
    ...
    for (let i = 0; i < trial.choices.length; i++) {
      ...
      html +=`
          <div
            class='jspsych-instructions-quiz-button'
            style='
              cursor: pointer;
              display: inline-block;
              margin: ${trial.margin_vertical} ${trial.margin_horizontal}
            '
            id='jspsych-instructions-quiz-button-${i}'
            data-choice='${i}'
          >
            ${str}
          </div>
    ...
    for (let i = 0; i < trial.choices.length; i++) {
      display_element.querySelector('#jspsych-instructions-quiz-button-' + i)
    ...
    const after_response = (choice) => {
      ...
      const btns = document.querySelectorAll(
          '.jspsych-instructions-quiz-button button',
      );
