...
// Training, Part 1: Practice selecting spaceships only
timeline.push({
  type: "two-step-instructions",
  stimulus: experiment.getStimuli().getImage("blackbackground.jpg"),
  leftStimulus: [],
  ...
  choices: [" "],
  prompt: [
    "Let's start by practicing how to select spaceships!",
    "",
    "You will see two spaceships on Earth. Choose one by pressing 'F' for left or 'J' for right.",
    "After you select a spaceship, you'll briefly see which planet it flies to.",
    "This will help you learn which spaceship goes to which planet.",
    "",
    "Click the red button and press Spacebar to start practicing spaceship selection.",
  ],
  include_score: false,
});
...
for (let i = 0; i < configuration.training.single; i++, trialNumber++) {
  timeline.push({
    ...
    prompt: [
      "Choose a spaceship!",
      "Press 'F' for the left spaceship or 'J' for the right spaceship.",
      "",
      i == 0 ? "Select your first spaceship!" : `Select spaceship ${i + 1}!`,
    ],
    responseWindow: configuration.timing.choice,
    isPractice: true,
  });
...
// Training, Part 2: Practice selecting aliens only
timeline.push(
  {
    type: "two-step-instructions",
    stimulus: experiment.getStimuli().getImage("blackbackground.jpg"),
    ...
    choices: [" "],
    prompt: [
      "Now let's practice selecting aliens!",
      "",
      "You will land directly on a planet with two aliens.",
      "Choose one by pressing 'F' for left or 'J' for right.",
      "You'll see if the alien shares space resources with you.",
      "",
      "Space resources look like this:",
    ],
    include_score: false,
  },
  {
    ...
    prompt: [
      ...
      "",
      "",
      "If the alien doesn't have space resources to share, you'll see an empty circle.",
      "The empty circle looks like this:",
      "",
      "Click the red button and press Spacebar to start practicing alien selection.",
    ],
    include_score: false,
  }
);
...
for (let i = 0; i < configuration.training.outcome; i++, trialNumber++) {
  timeline.push({
    ...
    rightStimulus: "tutalien4",
    responseWindow: configuration.timing.choice,
    isPractice: true,
  });
}

// Training, Part 3: Complete trials (spaceship + alien + reward)
timeline.push(
  {
    type: "two-step-instructions",
    stimulus: experiment.getStimuli().getImage("blackbackground.jpg"),
    ...
    choices: [" "],
    prompt: [
      "Now let's practice the complete mission!",
      "",
      "You will choose a spaceship on Earth, fly to a planet, then choose an alien.",
      "This is the full sequence you'll do in the real game.",
      "",
      "Pay attention to which spaceship takes you to which planet,",
      "and which aliens have more resources to share.",
      "",
      "Click the red button and press Spacebar to start complete mission practice.",
    ],
    include_score: false,
  }
);

// Create complete trial variables for practice
const completePracticeVariables = [];
for (let i = 0; i < configuration.training.both; i++) {
  completePracticeVariables.push({
    rightStimulus: "tutrocket2",
    leftStimulus: "tutrocket1",
    trialRow: i,
    trialNumber: trialNumber + i + 1,
  });
}

// Add complete practice trials using the existing createBlock function
timeline.push(createBlock(completePracticeVariables, practiceProbabilityData, true));
trialNumber += configuration.training.both;
...
// Practice trials use consistent rocket positions (no swapping)
